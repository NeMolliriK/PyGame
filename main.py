...
class Board:
    ...
    def up2(self):
        ...
        second_player.direction = 0
        second_player.rotate()
...
class Player(Sprite):
    ...
    def rotate(self):
        if 'first' in self.file:
            if self.direction == 0:
                self.image = pygame.image.load(f'data/first_player_up.png')
            elif self.direction == 1:
                self.image = pygame.image.load(f'data/first_player_left.png')
            elif self.direction == 2:
                self.image = pygame.image.load(f'data/first_player_down.png')
            elif self.direction == 3:
                self.image = pygame.image.load(f'data/first_player_right.png')
        else:
            if self.direction == 0:
                self.image = pygame.image.load(f'data/second_player_up.png')
            elif self.direction == 1:
                self.image = pygame.image.load(f'data/second_player_left.png')
            elif self.direction == 2:
                self.image = pygame.image.load(f'data/second_player_down.png')
            elif self.direction == 3:
                self.image = pygame.image.load(f'data/second_player_right.png')
...
def is_click(x, y, width, height):
    click = pygame.mouse.get_pressed()
    pos_mouse = pygame.mouse.get_pos()
    if click[0] == 1:
        if x <= pos_mouse[0] <= x + width:
            if y <= pos_mouse[1] <= y + height:
                return True
    else:
        return False


def print_text(text, x, y, color=(0, 0, 0), type='data/FONT_TYPE.ttf', size=40):
...
class Button():
    def __init__(self, x, y, width, height, bg, active_bg, text_x, text_y, text, color, size, round, action):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def update(self):
        if self.x < self.mouse[0] < self.x + self.width:
            if self.y < self.mouse[1] < self.y + self.height:
                pygame.draw.rect(screen, self.active_bg, (self.x, self.y, self.width, self.height),
                                 border_radius=self.round)
                if self.click[0] == 1:
        ...
        if self.text is not None:
            print_text(text=self.text, x=self.x + self.text_x, y=self.y + self.text_y, color=self.color,
                       size=self.size,
                       type=self.type)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_1 or event.key == pygame.K_2 or event.key == pygame.K_3:
                    ...
                    fn = "Player 1"  # ники
                    sn = "Player 2"
                    first_player = Player(first_players, x=board.left, y=board.top, file="first_player_right.png",
                                          lives=fl,
                                          nick=fn)
                    second_player = Player(second_players, x=1920 - 20 + board.left - board.cell_size,
                                           y=1080 - 30 + board.top - board.cell_size, file="second_player_right.png",
                                           lives=sl,
                                           nick=sn)
                    ...
                    while True:
                        ...
                        if is_click(x=950, y=900, width=170, height=70):  # close
                            terminate()
                        else:
                            pass
                        if is_click(x=800, y=900, width=130, height=70):
                            c = False
                            continue
                        else:
                            pass
